...
var blockUI = new KTBlockUI(target, {
  message: '<div class="blockui-message"><span class="spinner-border text-primary"></span> Mohon tunggu...</div>',
});
...
function date_initiate() {
  $('.date').each(function (i, obj) {
    $(this).datepicker({ format: "yyyy-mm-dd", clearBtn: true, language: "id", todayBtn: "linked", todayHighlight: true, autoclose: true, });
  });
}
...
function get_elemet_attribute(attribute_key, name, id, is_hide = false) {
  ...
  if (!is_hide) {
    attributnya = attribute[attribute_key].replace('name_atribute', `attribute|${name}`).replace('id_atribute', id).replace('hide', '')
  } else {
    attributnya = attribute[attribute_key].replace('name_atribute', `attribute|${name}`).replace('id_atribute', id).replace('hide', 'hidden')
  }
...
function to_currency_indo(value) {
  var str = value.toString().replace("", ""), parts = false, output = [], i = 1, formatted = null;
  if (str.indexOf(",") > 0) {
    parts = str.split(",");
    str = parts[0];
  }
...
function to_currency_indo(value, thousand_separator = '.', decimal_separator = ','){
  var str = value.toString().replace("", ""), parts = false, output = [], i = 1, formatted = null;
...
function from_currency_indo(formattedValue, thousand_separator = '.', decimal_separator = ',') {
...
function ShowIframeReportVerif(res, title, ev = null, return_element = false, id_kak) {
  if (ev != null) {
    ev.preventDefault()
  }
...
modal_sm = `<!--begin::Modal - Create App-->
                ...
                function showModal(e, { size = 'fullscreen', is_ajax = true, modal_content = '' }, jenis_modal = '') {
                  var url_load = "";
                  var title = "";
                  ...
                  url_load = linkLoad;
                  console.log(jenis_modal, 'jenis_modal');
                  ...
                  if (jenis_modal === 'arsip') {
                    $(`#${modal_id} .modal-header`).addClass('bg-primary  text-white');
                    console.log('arsip');
                } else {
                    $(`#${modal_id} .modal-header`).removeClass('bg-primary  text-white');
...
f (is_ajax) {
  modal_body.load(url_load, function () {
      ...
      modal_body.find('select').each(function () {
          $(this).select2({
              dropdownParent: $(`#${modal_id}`) // Pastikan dropdownParent benar
          });
          // Tambahkan inline style untuk meningkatkan z-index select2 container
          $(this).next('.select2-container').css('z-index', '1061'); // Nilai z-index harus lebih tinggi dari modal default (biasanya 1050)
      });

      // Inisialisasi DataTable setelah konten modal arsip dimuat
      if (modal_id === 'modal_extra_large' && $(`#${modal_body.attr('id')} #table_profil_arsip`).length > 0) {
          if (!window.table_arsip) {
              window.table_arsip = $(`#${modal_body.attr('id')} #table_profil_arsip`).DataTable();
          } else {
              window.table_arsip.destroy(); // Hancurkan instance sebelumnya jika ada
              window.table_arsip = $(`#${modal_body.attr('id')} #table_profil_arsip`).DataTable();
          }

          // Hubungkan search input setelah DataTable diinisialisasi
  ...
  modal_body.find('select').each(function () {
      ...
      // Tambahkan inline style untuk meningkatkan z-index select2 container
      $(this).next('.select2-container').css('z-index', '1061'); // Nilai z-index harus lebih tinggi dari modal default
  });
}
...
myModal_.addEventListener('shown.bs.modal', function () {
    try {
        ...
        // Penanganan error jika fungsi date_initiate tidak ada
    }
});